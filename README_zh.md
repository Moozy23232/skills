...
面向科研、源码学习与精简软件开发，可在不同 AI 编程工具中复用的 skills。
...
### [`paper-reading-coach`](.docs/paper-reading-coach_zh.md)

把一篇论文从快速定位读到可以验证的理解。它可以安排功能化阅读路线、补齐必要前置知识、拆解公式和图表、用小型可复现实验检查结论，并通过主动回忆暴露学习盲区。

### [`ponytail-lite`](.docs/ponytail-lite_zh.md)

用一套刻意精简的资深开发者工作方式，完成足够解决问题的最小方案。它会质疑推测性需求、复用现有实现、优先选择标准库和原生能力、修复根因，也可以检查 diff 中不必要的复杂度。
